...
-->
<template>
	<div class="stored-boards-chart-wrapper">
...
export default {
	...
	data() {
		return {
			...
			chartData: {
				...
				datasets: [
					{
						label: t('whiteboatd', 'Stored whiteboards'),
	...
	methods: {
		async fetchData() {
			this.loading = true
			try {
				const { data } = await axios.get(generateUrl('/apps/whiteboard/stats/boards-count'), {
					params: {
