	...
	useEffect(() => {
		...
		window.followUser = (userId: string) => {
			...
			if (!currentSocket?.connected) {
				console.warn('[Collaboration] Cannot follow user: Socket not connected')
				return
			}

			// Set the followed user ID in the collaboration store
			useCollaborationStore.setState({ followedUserId: userId })
			console.log(`[Collaboration] Recording agent now following user: ${userId}`)

			// Debug: Log current collaboration store state
			const state = useCollaborationStore.getState()
			console.log('[Collaboration] Current collaboration store state:', {
