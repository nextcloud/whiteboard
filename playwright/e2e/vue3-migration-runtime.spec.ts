...
import { createWhiteboard, openFilesApp, waitForCanvas } from '../support/utils.ts'
...
function trackConsoleErrors(page: Page) {
	...
	page.on('console', (message) => {
		if (message.type() === 'error') {
			...
			const isMissingUserStatus = text.includes('status of 404')
				&& message.location().url.includes('/ocs/v2.php/apps/user_status/api/v1/user_status')
			if (
...
test('Vue 3 personal settings save and failure', async ({ page }) => {
	...
	const successResponse = page.waitForResponse((response) => {
		return response.url().includes('/apps/whiteboard/settings/personal') && response.request().method() === 'POST'
	})
	...
	expect(consoleErrors).toEqual([
		expect.stringContaining('the server responded with a status of 500 (Internal Server Error)'),
	])
})
