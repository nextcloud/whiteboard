...
test('following a collaborator requests viewport sync', async ({ page, browser }) => {
	...
	const collaboratorAvatar = followerPage.locator('.UserList__collaborator--avatar-only .Avatar')
	await expect(collaboratorAvatar).toHaveCount(1, { timeout: 30000 })
	await collaboratorAvatar.first().click()
