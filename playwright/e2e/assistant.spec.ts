...
import { createWhiteboard, fetchBoardContent, getBoardAuth, openFilesApp } from '../support/utils'
...
test('Assistant Button', async ({ page }) => {
	...
	const auth = await getBoardAuth(page)
	...
	await expect.poll(async () => {
		...
		return content.elements.some((element) => element.text === DIAGRAM_DISCLOSURE_TEXT)
	}, {
		timeout: 30000,
		interval: 500,
	}).toBe(true)
	...
	const content = await fetchBoardContent(page, auth)
	const disclosureElement = content.elements.find((element) => element.text === DIAGRAM_DISCLOSURE_TEXT)
