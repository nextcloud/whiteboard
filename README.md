...
- Any credentials given on the first seed are applied to every discovered node, and so is TLS if the first seed uses `rediss://`
- A comma inside a password is not a separator: a single URL stays a single node
