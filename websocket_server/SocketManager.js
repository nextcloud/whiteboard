...
export default class SocketManager {
	...
	/**
	 * Handles client broadcasts to other clients in the room
	 * @param {Socket} socket - Socket.IO socket instance
	 * @param {string} roomId - Room identifier
	 * @param {ArrayBuffer} data - Broadcast data
	 * @param {Array} excludedSocketIds - Socket IDs to exclude from broadcast
	 */
	async handleClientBroadcast(socket, roomId, data, excludedSocketIds = []) {
		try {
			const userId = await this.getUserIdFromSocket(socket)
			Utils.logOperation(roomId, `Broadcasting data from user ${userId}`)

			// Broadcast to room without saving (clients handle persistence)
			this.broadcastToRoom(socket, roomId, data, excludedSocketIds)
		} catch (error) {
			Utils.logError(roomId, 'Error handling client broadcast', error)
		}
	}
